...
interface LovelaceDiffViewerProps {
...
export default function LovelaceDiffViewer({ backupFileContent, liveFileContent, fileName, backupFolderName, onRestore, onClose, isLoadingDiff }: LovelaceDiffViewerProps) {
    ...
    <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0, 0, 0, 0.6)', backdropFilter: 'blur(8px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 50, padding: '24px' }}>
      <div style={{ backgroundColor: '#2d2d2d', borderRadius: '16px', border: '1px solid rgba(255, 255, 255, 0.1)', boxShadow: '0 25px 50px -12px rgba(0, 0, 0, 0.5)', width: '100%', maxWidth: '1152px', maxHeight: '90vh', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
            ...
            <div>
              ...
              <p style={{ fontSize: '14px', color: '#9ca3af' }}>
                {!liveFileContent
                  ...
                  : 'Compare backup with current live version.'}              </p>
            </div>
            <button
              onClick={onClose}
        ...
        <main style={{ flex: 1, overflowY: 'auto', padding: '12px 24px' }}>
          <>
            {isLoadingDiff ? null : !liveFileContent ? (
              <div style={{ textAlign: 'center', padding: '0px 16px' }}>
                                  <div style={{
                                    ...
                                    alignItems: 'center',
                                    marginBottom: '16px'
                                  }}>                  <span>This file has been deleted. You can restore it from this backup version tho.</span>
                </div>
